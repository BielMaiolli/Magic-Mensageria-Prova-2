                ...
                cards.forEach(async (card) => {
                    ...
                    try {
                        const cardResponse = await fetch(
                            `${SCRYFALL_API}${encodeURIComponent(card)}`
                        );
                        if (!cardResponse.ok) {
                            throw new Error(
                                'Erro ao buscar a carta: ' + cardResponse.statusText
                            );
                        }
    ...
    async function fetchCommanderImage(commanderName) {
        try {
            const response = await fetch(
                `${SCRYFALL_API}${encodeURIComponent(commanderName)}`
            );
            if (!response.ok) {
                throw new Error(
                    'Erro ao buscar a carta do comandante: ' +
                        response.statusText
                );
            }
    ...
    async function deleteDeck(deckId) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(
                    'Erro ao excluir o deck: ' + errorData.message
                );
            }
    ...
    if (createDeckForm) {
        createDeckForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const response = await fetch(
                    `${API_URL}/deck/newDeckWithCommander?commanderName=${encodeURIComponent(
                        commanderName
                ...
                if (!response.ok) {
                    const errorData = await response.json();
                    throw new Error(
                        'Erro ao criar o deck: ' + errorData.message
                    );
                }
...
socket.on('notification', (data) => {
    if (!data) {
        console.error('Nenhum dado recebido na notificação');
        return;
    }

    console.log('Notificação recebida pelo WebSocket:', data);

    // Adiciona a notificação ao contêiner na tela
    let notificationsContainer = document.getElementById('notifications');
    if (!notificationsContainer) {
        notificationsContainer = document.createElement('div');
        notificationsContainer.id = 'notifications';
        notificationsContainer.style.position = 'fixed';
        notificationsContainer.style.bottom = '10px';
        notificationsContainer.style.right = '10px';
        notificationsContainer.style.width = '300px';
        notificationsContainer.style.backgroundColor = '#f9f9f9';
        notificationsContainer.style.border = '1px solid #ccc';
        notificationsContainer.style.borderRadius = '5px';
        notificationsContainer.style.padding = '10px';
        notificationsContainer.style.boxShadow = '0px 4px 6px rgba(0, 0, 0, 0.1)';
    ...
    notificationElement.style.backgroundColor = '#e3f7e3';
    notificationElement.style.borderLeft = '4px solid #4caf50';
    notificationElement.style.borderRadius = '3px';
    notificationElement.innerHTML = `
        <strong>Deck:</strong> ${data.deckName || 'N/A'} <br>
